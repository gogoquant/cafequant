...
from fdfs_client.base64 import *
...
class Fdfs_client(object):
    ...
    def get_file_info_ex(self, remote_file_id):
	    """
	    Get a file info from remote_file_id.
	    arguments:
	    @remote_file_id: string, file_id of file that is on storage server
	    @return dict {
		'create_timestamp': time_t,
		'file_size'       : file_size,
	    ...
	    tmp = split_remote_fileid(remote_file_id)
	    if not tmp:
		raise DataError('[-] Error: remote_file_id is invalid.(in get file info ex)')
	    
	    group_name, remote_filename = tmp
	    filename_len = len(remote_filename)
	    #import logging
	    #logging.error("group_name: %s, remote_filename: %s, filename_len: %d" % (group_name, remote_filename, filename_len))
	    
	    if (filename_len < FDFS_NORMAL_LOGIC_FILENAME_LENGTH):
		raise DataError("[-] Error: filename is too short, length: %d < %d" % 
		            (filename_len,FDFS_NORMAL_LOGIC_FILENAME_LENGTH))
	    
	    base_s = Base64()
	    remote_filename = remote_filename[FDFS_LOGIC_FILE_PATH_LEN:
		                              (FDFS_FILENAME_BASE64_LENGTH + FDFS_LOGIC_FILE_PATH_LEN)]
	    dest_list = base_s.base64_decode_auto(remote_filename)
	    ip_addr_int = base_s.buff2int(dest_list[0:4])
	    create_timestamp = base_s.buff2int(dest_list[4:-12])
	    file_size = base_s.buff2long(dest_list[8:-4])
	    
	    import socket
	    from struct import pack
	    if filename_len > FDFS_TRUNK_LOGIC_FILENAME_LENGTH or \
	       (filename_len > FDFS_NORMAL_LOGIC_FILENAME_LENGTH and ((filename_len & TRUNK_FILE_MARK_SIZE) == 0)):
		file_size = -1
		
	    return_dic = {
		"create_timestamp": create_timestamp, 
		"file_size": file_size,
		"source_ip_addr": socket.inet_ntoa(pack('!L', ip_addr_int))
	    }
	    return return_dic
    
    def get_file_info(self, remote_file_id):
        '''
        Get a file info from storage server.
        arguments:
        @remote_file_id: string, file_id of file that is on storage server
        @return dict {
            'create_timestamp': time_t,
        ...
        tmp = split_remote_fileid(remote_file_id)
        if not tmp:
            raise DataError('[-] Error: remote_file_id is invalid.(in get file info)')
        group_name, remote_filename = tmp
        tc = Tracker_client(self.tracker_pool)
        store_serv = tc.tracker_query_storage_stor_with_group(group_name)
        store = Storage_client(store_serv.ip_addr, store_serv.port, self.timeout)
        return store.storage_query_file_info(tc, store_serv, remote_filename)
	...
	print store_serv.ip_addr, store_serv.port
        store = Storage_client(store_serv.ip_addr, store_serv.port, self.timeout)
