...
class TopicCallback:
    ...
    def topic_callback_request(self, msg):
        ...
        if state == "204 FAILED":
            ...
            db = db_conn.training
            db.groups.update({"groupname": groupname},
                             {"$inc": {
                                 "training_fail_count": 1
                             }})
            ...
            for i in xrange(9):
                ...
                if os.path.exists(new_file_temp):
                    ...
                    consumer.add_method(new_req_id,
                                        self.search_callback_request)
                    with open(new_file_temp, "rb") as file_data:
                        binary_content = file_data.read()
                    search_msg = Message(
                        binary_content,
                        delivery_mode=2,
                        reply_to=options.Search_CallBack_Queue,
                        headers={
                            "Method": "Search",
    ...
    def search_callback_request(self, msg):
        ...
        if not recs_dic.has_key(cur_req_id):
            ...
            return
        if state == "204 FAILED":
            recs_dic[cur_req_id]["result_map"][cur_index] = [
                key_num, index_map[cur_index], 0
            ]
        else:
            ...
            if image_path == "204 FAILED":
                ...
                try:
                    ...
                    if file_md5 == self.origin_md5:
                        recs_dic[cur_req_id]["result_map"][cur_index] = [
                            key_num, index_map[cur_index], image_path_list[2]
                        ]
                    else:
                        recs_dic[cur_req_id]["result_map"][cur_index] = [
                            key_num, index_map[cur_index], -1
                        ]
                except IndexError:
        ...
        if len(recs_dic[cur_req_id]
               ["result_map"].values()) == recs_dic[cur_req_id]["count"]:
            ...
            for key_same in recs_dic[cur_req_id]["result_map"].values():
                if key_same[0] == 0 or key_same[2] == 0:
                    ...
                    key_rate = min(
                        max(key_same[0] * 9 / total_key_count, 0.6), 1.2)
                    same_rate = min(max(float(key_same[2]) / 30, 0.5), 1.3)
                cur_rate = key_rate * same_rate * key_same[1]
                logging.error("key_rate: %s, same_rate: %s, cur_rate: %s" %
                              (key_rate, same_rate, cur_rate))
                rate_value = rate_value + cur_rate

            rate_value = min(max(float(total_key_count) / 600, 0.6),
                             1.1) * rate_value
    ...
    def update_res_picture(self, similarity):
        ...
        if not hasattr(self, 'keynumber'):
            self.keynumber = 0
        db = db_conn.resource
        query = {'md5': self.origin_md5, 'delete_flag': {'$ne': '1'}}
...
def on_amqp_connection():
    ...
    ch = conn.channel()
    ch.exchange_declare(
        exchange=options.Search_CallBack_Exchange, type="topic", durable=True)
    ...
    ch.queue_bind(
        queue=options.Search_CallBack_Queue,
    ...
    logging.error('consume:[E %s][Q %s][R %s]' %
                  (options.Search_CallBack_Exchange,
                   options.Topic_CallBack_Queue, options.Topic_CallBack_Queue))
    logging.error(
        'consume:[E %s][Q %s][R %s]' %
        (options.Search_CallBack_Exchange, options.Search_CallBack_Queue,
         options.Search_CallBack_Queue))
...
mongo_host = 'mongodb://%s:%s@%s' % (setting.MONGO_USER, setting.MONGO_PASS,
                                     setting.MONGO_HOST)
...
options['Search_CallBack_Queue'].set(
    options.Queue_Topic_Callback_Search_Callback_Queue)
