...
def tudou_download_by_iid(iid, title, output_dir = '.', merge = True, info_only = False, useragent=None):
    data_string = get_decoded_html('http://www.tudou.com/outplay/goto/getItemSegs.action?iid=%s' % iid, useragent=useragent)
...
def tudou_download_by_id(id, title, output_dir = '.', merge = True, info_only = False, useragent=None):     
    html = get_html('http://www.tudou.com/programs/view/%s/' % id, useragent=useragent)
    
    iid = r1(r'iid\s*[:=]\s*(\S+)', html)    
    title = r1(r'kw\s*[:=]\s*[\'\"]([^\']+?)[\'\"]', html)
    tudou_download_by_iid(iid, title, output_dir = output_dir, merge = merge, info_only = info_only)

def tudou_download(url, output_dir = '.', merge = True, info_only = False, useragent=None):
    # Embedded player
    id = r1(r'http://www.tudou.com/v/([^/]+)/', url)
    if id:
        return tudou_download_by_id(id, title="", info_only=info_only, useragent=useragent)
    ...
    if not iid:
        return tudou_download_playlist(url, output_dir, merge, info_only, useragent=useragent)
    
    result_urls = []
    result_urls.append(tudou_download_by_iid(iid, title, output_dir = output_dir, merge = merge, info_only = info_only, useragent=useragent))
    return result_urls
...
def parse_playlist(url, useragent=None):
    ...
    return [(atitle + '-' + x['title'], str(x['itemId'])) for x in json.loads(get_html(url, useragent=useragent))['message']]

def tudou_download_playlist(url, output_dir = '.', merge = True, info_only = False, useragent=None):
    ...
    for i, (title, id) in enumerate(videos):
        ...
        url_list.append(tudou_download_by_iid(id, title, output_dir = output_dir, merge = merge, info_only = info_only, useragent=useragent))
    return url_list
