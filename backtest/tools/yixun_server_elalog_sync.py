...
class YiXinServerElalogSync(object):
    ...
    def run(self):
        ...
        batch_start_time, batch_end_time = self.get_ela_query_time(self.day_s, self.day_e)
            ...
            "query": {
                "bool": {
                    "must": [
                        {"bool": {
                            "should": [
                                {"regexp": {"log": scan_info_pattern}},
                                {"regexp": {"log": h5_share_read_pattern}},
                                {"regexp": {"log": video_share_read_pattern}},
                                {"regexp": {"log": ar_create_browser_pattern}},
        ...
        # 统计log记录中的识别总数
        res_stats_records, imei_records, browser_stats_records = yield tornado.gen.Task(self._make_data_records, log_data)
        res_stats_list = yield tornado.gen.Task(self.make_records_to_list,
                                                res_stats_records)

        yield tornado.gen.Task(self.res_statistics_service.server_insert_or_update_list, res_stats_list)
        ...
        logging.error("[*] sync_info send doc:%s" % simplejson.dumps(res_stats_list))
        logging.error("sync_info send imei list doc:%s" % simplejson.dumps(imei_records))
        logging.error("sync_info ar create browser list doc:%s" % simplejson.dumps(browser_stats_records))
    ...
    def get_ela_query_time(self, start_time, end_time):
        ...
        start_time_timestamp = time_common.string_to_timestamp(start_time,
                                                               time_common.FULL_PATTERN) - time_common.hour_seconds*8
        end_time_timestamp = time_common.string_to_timestamp(end_time,
                                                             time_common.FULL_PATTERN) - time_common.hour_seconds*8

        batch_start_time = time_common.timestamp_to_string(start_time_timestamp, date_pattern)
        batch_end_time = time_common.timestamp_to_string(end_time_timestamp, date_pattern)
    ...
    def get_logging_data(self, query, callback=None):
        log_list = []
        log_total = yield tornado.gen.Task(self.ela_logging_service.get_logging_total, query)
        if not log_total:
            logging.info('Not searching for the record')
        else:
            query['size'] = log_total
            log_list = yield tornado.gen.Task(self.ela_logging_service.get_logging_list, query)
    ...
    def update_imei_list(self, imei_records, callback=None):
        ...
        for imei, imei_data in imei_records.items():
            ...
            old_imei = yield tornado.gen.Task(self.imei_m.find_one, {"imei": imei})
            if old_imei:
                ...
                yield tornado.gen.Task(self.imei_m.update, {"imei": imei}, {"$set": update_data})
    ...
    def update_browser_stats(self, browser_stats_records, callback=None):
        ...
        yield tornado.gen.Task(self.browser_s.insert_or_update_browser, browser_stats_list)
        callback(None)
    ...
    def _get_network_area(self, client_ip, callback=None):
        try:
            network = ''
            area = ''
            ip_info = yield tornado.gen.Task(self.ip_s.select_ip_info, client_ip)
            if ip_info:
    ...
    def match_scan_record(self, match_info, res_stats_records, callback=None):
        ...
        stats_date = time_common.string_to_timestamp(record_date, time_common.DAY_PATTERN)
        ...
        if md5 and imei:
            ...
            area = ''
            if client_ip:
                network, area = yield tornado.gen.Task(self._get_network_area, client_ip)
            ...
            imei_record = record['imei_record_map'].get(imei, {"system": "", "network": {}, "area": {}})
    ...
    def match_h5_share_read(self, match_info, res_stats_records, callback=None):
        ...
        md5 = ""
        if resource_id:
            res_pic = yield tornado.gen.Task(self.resource_picture_service.one, resource_id=resource_id)
            if res_pic:
                md5 = res_pic.get("md5", "")
    ...
    @tornado.gen.engine
    def match_video_share_read(self, match_info, res_stats_records, callback=None):
        ...
        if video:
            ...
            if resource_id:
                resource_pic = yield tornado.gen.Task(self.resource_picture_service.one, resource_id=resource_id)
                if resource_pic:
    ...
    @tornado.gen.engine
    def match_ar_create_browser(self, match_info, browser_stats_records, callback=None):
    ...
    def _make_data_records(self, logo_data, callback=None):
        ...
        for log in logo_data:
            if scan_info_pattern.match(log):
                ...
                res_stats_records = yield tornado.gen.Task(self.match_scan_record,
                                                           match_info,
                                                           res_stats_records)
            if h5_share_read_pattern.match(log):
                info = h5_share_read_pattern.match(log)
                match_info = eval(info.group(1))
                res_stats_records = yield tornado.gen.Task(self.match_h5_share_read,
                                                           match_info,
                                                           res_stats_records)
            ...
            if video_share_read_pattern.match(log):
                ...
                match_info = eval(info.group(1))
                res_stats_records = yield tornado.gen.Task(self.match_video_share_read,
                                                           match_info,
                                                           res_stats_records)
            if ar_create_browser_pattern.match(log):
                info = ar_create_browser_pattern.match(log)
                match_info = eval(info.group(1))
                browser_stats_records = yield tornado.gen.Task(self.match_ar_create_browser,
                                                               match_info,
        ...
        for k, v in res_stats_records.items():
            ...
            for imei, imei_value in v['imei_record_map'].items():
                imei_info = imei_records.get(imei, {'system': "", 'network': {}, 'area': {}})
    ...
    def make_records_to_list(self, res_stats_records, callback=None):
        res_stats_list = []
        for k, v in res_stats_records.items():
            res_pic = yield tornado.gen.Task(self.resource_picture_service.one, md5=v.get('md5'))
            if res_pic:
                v['resource_id'] = res_pic['resource_id']
                v['resource_picture_id'] = res_pic['resource_picture_id']
                query = {
                    "resource_id": res_pic['resource_id']
                }

                # start_time = v['stats_date'] - time_common.day_seconds
                # end_time = v['stats_date']

                start_time = time_common.string_to_timestamp(self.day_s, time_common.FULL_PATTERN)
                end_time = time_common.string_to_timestamp(self.day_e, time_common.FULL_PATTERN)

                query["last_modify"] = {"$gte": start_time, "$lt": end_time}
                logging.error(query)
                # 获取统计时间的收藏数和评论
                v['collection'] = yield tornado.gen.Task(self.collection_m.count, query.copy())
                v['comment'] = yield tornado.gen.Task(self.comment_m.count, query.copy())
