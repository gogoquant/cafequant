...
def iqiyi_download(url, output_dir = '.', merge = True, info_only = False):
    ...
    cmd_js = "nodejs ./iqiyi_parse.js %s %s %s %s" %(url, tvId, sea_nums, sbase_nums)
    ...
    assert sc
    assert src
    metal_url = 'http://cache.m.iqiyi.com/jp/tmts/%s/%s/?type=mp4&sc=%s&src=%s&t=%s&__jsT=%s' % (tvId, videoId, sc, src, t, jsT)
    ...
    temp_url = "http://data.video.qiyi.com/%s" % mp4_url.split("/")[-1].split(".")[0] + ".ts"
    try:
