...
import time
from views.web.permission import Permission
...
class WebHandler(RequestHandler):
    ...
    def render(self, **kwargs):
        ...
        # 设置导航栏信息
        # nav_active = self.__class__.nav_active \
        # if hasattr(self.__class__, 'nav_active') else None
    ...
    def navbar_list(self):
        '''头部导航列表'''
        nav_list = [
            {"name": "index", "title": self._(
                "topic list"), "href": "/web/resource"},
            {"name": "infoview", "title": self._(
                "Statics"), "href": "/web/infoview/group"},
        ]
        return nav_list
...
class AdminHandler(WebHandler):
    ...
    """管理员Handler, 使用账号密码交互的特殊接口"""

    def check_pwd(self):
        config = Config.GetConfig()
        user = self.get_argument('admin_user')
        pwd = self.get_argument('admin_pwd')
        if user != config.get('admin_user'):
            return False
        if pwd != config.get('admin_pwd'):
            return False
...
class WebAsyncAuthHandler(WebHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        # 获取session中的用户属性
        self.editorinfo = yield tornado.gen.Task(self.session.get,
                                                 'editorinfo')
                ...
                if login_time != login_time_cookie:
                    ...
                    if editor_id is None:
                        ...
                        yield tornado.gen.Task(self.session.set, 'editorinfo',
                                               editorinfo)
                        logging.info(
                            '--INFO-- WebAsyncAuthHandler.check_priv \
                            recreate session'
                        )
