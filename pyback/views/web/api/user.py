...
class AdminLoginHandler(WebHandler):
    ...
    def _post_(self):
        ...
        user_id, editorinfo = yield tornado.gen.Task(self.user_s.login,
                                                     email,
                                                     passwd,
                                                     token,
                                                     token_from,
        ...
        if editorinfo == PasswdErrorException:
            ...
            # set time cookie
            self.set_cookie(EDITOR_LOGIN_TIME_COOKIE_KEY,
                            "%d" % login_time,
                            expires=login_expires)

            # set auth cookie and session rebuilder
            login_session = "%d|%s|%s " % (login_time, email,
                                           editorinfo.get('passwd', ''))
            self.set_secure_cookie(EDITOR_SESSION_COOKIE_KEY,
                                   login_session,
                                   expires_days=None)
...
class ForgetPasswdHandler(WebAsyncAuthHandler):
    ...
    def _get_(self):
        ...
        change_passwd_url = setting.SITE_URL + \
            "/user/change_passwd/%s" % (reset_passwd_id)

        import os.path
        import sys
        find_passwd_path = os.path.join(
            os.path.dirname(__file__), "../../find_passwd.tpl")
...
class ForgetPasswdHandler(WebAsyncAuthHandler):
    ...
    def _post_(self, reset_passwd_id):
        ...
        reset_passwd = yield tornado.gen.Task(
            self.user_service.get_reset_passwd_by_id, reset_passwd_id)
        if not reset_passwd:
            self.render_error(msg=self._('No reset passwd session'),
                              status=203)
            return
        ...
        if user_password != comfirm_password:
            self.render_error(
                msg=self._('Confirm password is different from new password!'),
                status=203)
            return
...
class UserInfoHandler(WebAsyncAuthHandler):
    ...
    def _get_(self, user_id):
        ...
        user_info = yield tornado.gen.Task(self.user_s.info,
                                           None,
                                           user_id,
                                           self_user_id,
                                           viewMore=True)
