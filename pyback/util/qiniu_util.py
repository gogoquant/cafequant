...
# 如果文件大小大于4m，自动断点续传
# 如果文件名已存在将返回错误
def upload_by_path(token,
                   file_path,
                   filename=None,
                   socket_proxy_host=None,
                   socket_proxy_port=None):

    if socket_proxy_host and socket_proxy_port:
        import socks
        import socket
        socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS5, socket_proxy_host,
                              socket_proxy_port)
    ...
    ret, info = qiniu.put_file(
        token, filename, file_path, mime_type=content_type, check_crc=True)
    if socket_proxy_host and socket_proxy_port:
...
def upload_by_stream(token, file_name, data, file_size=None, file_key=None):
    ...
    ret, info = qiniu.put_data(
        token, file_key, data, mime_type=mime_type, check_crc=True)
    return ret, info
...
if __name__ == '__main__':
    ...
    ret, info = upload_by_path(
        token, path, file_size, filename="testupload.mp4")
    logging.error("ret:%s" % ret)
    logging.error("info:%s" % info)

    # upload by stream
    with open(path, 'rb') as f:
        ret, info = upload_by_stream(
            token, filename, f.read(), file_size=file_size)
        logging.error("ret:%s" % ret)
        logging.error("info:%s" % info)
