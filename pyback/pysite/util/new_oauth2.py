...
#
# WECHAT_MP_APP_ID = "wx7934706afe636b00"
# WECHAT_MP_SECRET = "09c549a3b8d6f551c48e0c68e4fdabee"

WEBCHAT_OPEN_APP_ID = "wx537feebd640931cc"
WEBCHAT_OPEN_SECRET = "f1e54b2d0da8ea5ac5bcb6a8c7a8cf79"
...
def is_in_yixun_webview(user_agent):
    if user_agent.lower().find('yixun') > -1:
        return True
    else:
        return False


def is_in_wechat_webview(user_agent):
    if user_agent.lower().find("micromessenger") > -1:
        return True
    else:
        return False
...
class Webchat(OAuth2):
    ...
    @tornado.gen.engine
    def get_signature_for_jsapi(self, url, callback):
        current_time = int(time.time())
        jsapi_ticket = yield tornado.gen.Task(self.get_apiticket_for_jsapi)
        string1 = "jsapi_ticket=" + jsapi_ticket + "&noncestr=" + NONCESTR + "&timestamp=" + str(
            current_time) + "&url=" + url
