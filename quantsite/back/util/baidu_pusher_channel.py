...
from util.baidu_pusher_core import *
...
class Channel(object):
    ...
    def __init__(self, apiKey, secretKey, arr_curlOpts=None):
        ...
        self._method_channel_in_body = [
            'push_msg', 'set_tag', 'fetch_tag', 'delete_tag', 'query_user_tags'
        ]
        if (not isinstance(self._curlOpts, dict)):
            raise ChannelExcepthion('invalid param -arr_curlopt is not an dict',
                                    Channel.CHANNEL_SDK_INIT_FAIL)
    ...
    def pushMessage(self, push_type, messages, message_keys, optional=None):
        ...
        try:
            ...
            arrArgs = self._mergeArgs(
                [Channel.PUSH_TYPE, Channel.MESSAGES, Channel.MSG_KEYS],
                tmpArgs)
            ...
            if (push_type == Channel.PUSH_TO_USER):
                ...
                if (not arrArgs.has_key(Channel.TAG_NAME) or
                    ...
                    raise ChannelException(
                        "tagName should be specified in optional when pushType is PUSH_TO_TAG",
                        self.CHANNEL_SDK_PARAM)
            elif (push_type == Channel.PUSH_TO_ALL):
                pass
            else:
                raise ChannelException("pushType value is invalid",
                                       self.CHANNEL_SDK_PARAM)

            arrArgs[Channel.DEVICE_TYPE] = optional[Channel.DEVICE_TYPE]
            arrArgs[Channel.DEPLOY_STATUS] = optional[Channel.DEPLOY_STATUS]
            arrArgs[Channel.MESSAGE_TYPE] = optional[Channel.MESSAGE_TYPE]
            arrArgs[Channel.PUSH_TYPE] = push_type
            if (isinstance(arrArgs[Channel.MESSAGES], list)):
                arrArgs[Channel.MESSAGES] = json.dumps(
                    arrArgs[Channel.MESSAGES])
            if (isinstance(arrArgs[Channel.MSG_KEYS], list)):
                arrArgs[Channel.MSG_KEYS] = json.dumps(
                    arrArgs[Channel.MSG_KEYS])
            return self._commonProcess(arrArgs)
    ...
    def deleteMessage(self, userId, msgId, optional=None):
        ...
        try:
            ...
            arrArgs = self._mergeArgs([Channel.USER_ID, Channel.MSG_IDS],
                                      tmpArgs)
    ...
    def _checkString(self, string, minLen, maxLen):
        if (isinstance(string, str) and len(string) >= minLen and
                len(string) <= maxLen):
            return True
        else:
            return False
    ...
    def _adjustOpt(self, opt):
        if (not ((opt is not None) and isinstance(opt, dict))):
            raise ChannelException('no params are set',
                                   Channel.CHANNEL_SDK_PARAM)
    ...
    def _baseControl(self, opt):
        ...
        if (opt.has_key(Channel.CHANNEL_ID)):
            if (opt[Channel.CHANNEL_ID] is not None and
                    opt[Channel.METHOD] not in self._method_channel_in_body):
        ...
        # request = RequestCore(url, '10.0.1.32:31287')
    ...
    def _mergeArgs(self, arrNeed, tmpArgs):
        ...
        if (len(tmpArgs) - 1 != len(arrNeed) and len(tmpArgs) != len(arrNeed)):
            ...
            raise ChannelException(
                'invalid sdk, params, params' + keys + 'are need',
                Channel.CHANNEL_SDK_PARAM)
        if (len(tmpArgs) - 1 == len(arrNeed) and tmpArgs[-1] is not None and
            (not isinstance(tmpArgs[-1], dict))):
            raise ChannelException(
                'invalid sdk params, optional param must bean dict',
                Channel.CHANNEL_SDK_PARAM)
        ...
        if (isinstance(arrNeed, list)):
            for key in arrNeed:
                if (tmpArgs[idx] is None):
                    raise ChannelException('lack param ' + key,
                                           Channel.CHANNEL_SDK_PARAM)
    ...
    def _channelExceptionHandler(self, ex):
        print ex.error_msg
        print self._arrayErrorMap.get(ex.error_code, "")
