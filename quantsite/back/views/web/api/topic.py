...
class TopicAddHandler(WebAsyncAuthHandler):
    ...
    def _post_(self):
        ...
        topic_id = yield tornado.gen.Task(self.topic_s.add, title=title, tag=tag, content=content, user_id=user_id, brief=brief)

        #构建跳转地址
        redir_url = setting.MISAkA_STATIC_URL + "/web/topic?topic_id=" + topic_id

        #新文章发布消息
        pub_id = yield tornado.gen.Task(self.pub_s.add, title=title, brief=brief, content=None, \
                user_id=user_id, img_url=None, redir_url=redir_url, \
                user_to=None, msg_type="active")
...
class TopicListHandler(WebHandler):
    ...
    def _post_(self):
        ...
        logging.info("pos %s count %s tag_id %s user_id %s" % (pos,count, tag_id, user_id) )

        #使用session中的用户id
        if not user_id is None:
            user_id = self.editorinfo["user_id"]
        
        topics = yield tornado.gen.Task(self.topic_s.get_list, tag_id=tag_id,\
                user_id=user_id, title=title, file_format=file_format, pos=pos, count=count, sort_key = sort_key)
        
        for topic in topics:
            user_info = yield tornado.gen.Task(self.user_s.get, user_id = topic["user_id"], name=None, email=None)
...
class TopicUpdateHandler(WebAsyncAuthHandler):
    ...
    def _post_(self):
        ...
        yield tornado.gen.Task(self.topic_s.update, topic_id, user_id, title, brief, content, tag_id, file_format)
