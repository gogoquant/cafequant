...
class WebAsyncAuthHandler(WebHandler):
    ...
    def make_editor_login_info(self, editorinfo, callback=None):
        ...
        login_session = "%d|%s|%s " % (login_time, editorinfo["editor_name"], editorinfo['editor_password'])
    ...
    def is_session_expired(self, callback=None):  
        ...
        if user_id and user_session and user_id == user_session.get("user_id", ''):
                ...
                if login_time is None:
                    ...
                    self.set_cookie(EDITOR_LOGIN_TIME_COOKIE_KEY, login_time, expires=login_expires)
    ...
    def check_priv(self, editorinfo, callback=None):
        ...
        if not editorinfo:
            try:  
                # 尝试重建session回话
                login_time_lst = self.get_cookie(EDITOR_LOGIN_TIME_COOKIE_KEY).split('|')
                login_time = int(login_time_lst[0])

                session_lst = self.get_secure_cookie(EDITOR_SESSION_COOKIE_KEY).split('|')
                ...
                if login_time != login_time_cookie:
                    ...
                    editor_id, editor = yield tornado.gen.Task(editor_service.login, editor_name_cookie, editor_password_md5)
                ...
                import traceback
                traceback.print_exc()
                logging.error('--ERROR-- WebAsyncAuthHandler.check_priv error, e=%s' % str(e))
                has_priv = False
